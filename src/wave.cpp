...
Wave::Wave(const std::string& filename) throw(std::exception) {
    ...
    if (data.dataID[0] == 'f' && data.dataID[1] == 'a' &&
        data.dataID[2] == 'c' && data.dataID[3] == 't') {
...
Wave::Wave(int16_t nChannels, int32_t nSamplesPerSec,
           int16_t wBitsPerSample) throw(std::exception) {
...
Wave Wave::operator+(const Wave& w) const throw(std::exception) {
    if (fmt.wFormatTag != w.fmt.wFormatTag)
        throw std::runtime_error(
                "Can't concatenate waves with different format tags");
...
void Wave::get_samples(unsigned int offset, unsigned int size,
                       std::vector<char>& samples) const {
    if (offset > (unsigned) data.dataSIZE)
        return;

    unsigned int real_size = (offset + size) < (unsigned) data.dataSIZE ? size :
                             (unsigned) data.dataSIZE - offset;
...
void Wave::append_samples(const std::vector<char>& samples_l,
                          const std::vector<char>& samples_r) {
    if (fmt.nChannels != 2)
    ...
    if (samples_l.size() != samples_r.size())
        throw std::logic_error((
                                       ...
                                       std::to_string(samples_l.size()) +
                                       " r " + std::to_string(
                                               samples_r.size())).c_str());
    ...
    for (size_t i = 0; i < size; i = i + bytes_per_sample) {
        wave_.insert(wave_.end(), samples_l[i],
                     samples_l[i + bytes_per_sample]);
        wave_.insert(wave_.end(), samples_r[i],
                     samples_r[i + bytes_per_sample]);
    }
...
void
Wave::append_samples(const char* samples_l, const char* samples_r, int size) {
    if (fmt.nChannels != 2)
        throw std::logic_error((
                                       "Wave::append_samples(): cannot add stereo samples, nChannels = " +
                                       std::to_string(fmt.nChannels)).c_str());
